...
export default function AbuseTimeline() {
  ...
  const counts = useMemo(() => {
    ...
    ;(data || []).forEach(e => { if (c[e.decision] != null) c[e.decision]++ })
    return c
  }, [data])

  const filtered = useMemo(() => {
    if (!data) return []
    return filter === 'ALL' ? data : data.filter(e => e.decision === filter)
  }, [data, filter])
    ...
    <div className="space-y-6 animate-fade-in">
      <header className="flex items-start justify-between">
        <div>
          ...
          <p className="text-slate-400 mt-1">Every flagged, slowed, or blocked request — your security audit trail</p>
        </div>
      ...
      <ChartCard title="Events" subtitle={`${filtered.length} of ${data?.length ?? 0} flagged events`}>
        ...
        {filtered.length === 0 ? (
          <div className="py-16 text-center text-slate-500">
            ...
            <p className="text-sm">No abuse events yet — try the Attack Simulator to generate some</p>
          </div>
        ) : (
...
function EventRow({ event }) {
  return (
    <div className="flex items-start gap-4 p-4 rounded-xl bg-bg-soft border border-slate-800/60
                    hover:border-slate-700 transition-all animate-slide-up">
      <DecisionBadge value={event.decision} />
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-3 flex-wrap">
          <span className="text-sm font-medium text-slate-100">{event.userName || 'anonymous'}</span>
