...
export default function About() {
  return (
    <div className="space-y-6 animate-fade-in max-w-5xl">
      <header>
        <h1 className="text-3xl font-bold tracking-tight text-text-primary">About This Project</h1>
        <p className="text-text-secondary mt-1">
          Intelligent API Usage Monitoring and Abuse Behavior Detection System
        </p>
      </header>

      <div className="card border-brand/30 bg-gradient-to-br from-brand-pale via-bg-card to-bg-card">
        <div className="flex items-start gap-5">
          <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-brand to-brand-dark flex items-center justify-center glow flex-shrink-0">
            <ShieldCheck className="w-8 h-8 text-white" strokeWidth={2.5} />
          </div>
          <div>
            <h2 className="text-2xl font-bold mb-2 text-text-primary">API Guardian</h2>
            <p className="text-text-secondary leading-relaxed">
              ...
              Combines a <strong>rule engine</strong> with an <strong>ML deep-learning model</strong>
      ...
      <ChartCard title="Detection pipeline" subtitle="Every request passes through 5 stages">
        <div className="grid grid-cols-1 md:grid-cols-5 gap-3">
          ...
          <Stage n={4} title="ML Score"  text="Model scores behavioral sequences for anomalies." />
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <ChartCard title="Tech Stack">
          <div className="space-y-3">
            ...
            <StackItem icon={Cpu}      label="ML Service" text="Python · FastAPI · PyTorch" />
        ...
        <ChartCard title="ML Model">
          <div className="space-y-3 text-sm">
            <Row k="Architecture"     v="Behavioral deep learning" />
            <Row k="Output"           v="Sigmoid → probability score" />
            <Row k="Loss"             v="Binary cross-entropy" />
      ...
      <ChartCard title="Team" subtitle="Minor Project · PR1103 · JK Lakshmipat University">
        ...
        <div className="mt-6 flex items-center gap-3 text-sm text-text-secondary border-t border-bg-border pt-4">
          <GraduationCap className="w-4 h-4" />
          Faculty Supervisor: <span className="text-text-primary font-medium">Prof. Devendra Bhavsar</span>
...
function Item({ icon: Icon, title, text }) {
  return (
    <div className="flex gap-3 p-3 rounded-lg bg-bg-soft border border-bg-border">
...
function Stage({ n, title, text }) {
  return (
    <div className="p-4 rounded-xl bg-bg-soft border border-bg-border relative overflow-hidden">
      <div className="absolute top-2 right-3 text-5xl font-black text-text-muted/20">{n}</div>
      <h4 className="font-semibold text-text-primary relative">{title}</h4>
      <p className="text-xs text-text-muted mt-1 relative">{text}</p>
    </div>
  )
}
...
function Row({ k, v }) {
  return (
    <div className="flex items-center justify-between py-1.5 border-b border-bg-border last:border-0">
      ...
      <span className="text-text-primary mono text-xs">{v}</span>
    </div>
  )
}
...
function Member({ name, id, role }) {
  return (
    <div className="p-4 rounded-xl bg-bg-soft border border-bg-border text-center">
      <div className="w-14 h-14 mx-auto mb-3 rounded-full bg-gradient-to-br from-brand-light to-brand flex items-center justify-center text-lg font-bold text-white">
        {name.split(' ').map(n => n[0]).join('').slice(0, 2)}
      </div>
      <h4 className="font-semibold text-text-primary">{name}</h4>
      <p className="text-xs mono text-text-muted mt-0.5">{id}</p>
      <p className="text-xs text-brand mt-2 font-semibold uppercase tracking-wider">{role}</p>
    </div>
  )
}